...
// current y cordinate of pig
int *ptr_y = new int;
int x = 100;  // x coordinate(only used for adjustment)
int yrecord =
    200;  // y cordinate of the pig when up was pressed (used for the equation)
...
int Screennumber =
    1;  // screen number 1 is home screen screen number 2 is playing screen
int start = 0;  // used to determine if the play has clicked pressed up and the
                // pig will start flying
int died = 0;   // turs to 1 when the pig dies
int score = 0;  // used to count the score
int recentscore = 0;
float highscore = 0;
int number = 0;  /// the number that will be displayed while playing the game in
                 /// the ones column
  ...
  *ptr_y = 200;
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(screen)) {
      ...
      if (Screennumber == 2) {
        if (timer2 > repressDelay) {  // checks when up was last pressed(used so
                                      // you dont double jump)
          if (screen.key.code == Keyboard::Up) {  // up input
            ...
            yrecord = *ptr_y;  // record y cordinate when up was pressed
          }
        }
      }
    }
    if (Screennumber == 1) {
      if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
        ...
        c = 1;  // c is used to tell the code that the mouse has been clicked
                // and it needs to look as the coordinates
      }
    }
    ///////Tick
    ...
    if (pipe_x1 > 10 && pipe_x1 < 200 &&
        (*ptr_y > pipe_y1 - 80 || *ptr_y < pipe_y3 - 20)) {
      if (died == 0) {
        died = 1;
        timer4 = 0;
      }
    ...
    if (pipe_x2 > 10 && pipe_x2 < 200 &&
        (*ptr_y > pipe_y2 - 80 || *ptr_y < pipe_y4 - 20)) {
      if (died == 0) {
        died = 1;
        timer4 = 0;
      }
