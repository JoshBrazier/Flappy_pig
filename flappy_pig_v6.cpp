...
/////clock
    float timer=0,timer2=0.5,timer3=0,timer5=0,delay=0.016,pipe_timer=0; 
...
//////inputs
            ...
            if (Screennumber==2){
                if (timer2>repressDelay){ //checks when up was last pressed(used so you dont double jump)
                    if (screen.key.code==Keyboard::Up){ //up input
                        ...
                        if (c==0){//determines if this is the first jump
                            c=1;
                            yrecord=200;
                        }
    ...
    if (timer>delay)//decides if screen needs to be refreshed
      {
        if (y<752 && c==1){//checks if pig has hit the ground
        y=yrecord-((-1200)*(timer2*timer2)+(600*timer2));//uses an parabola to calculate where the pig should be
        }
        else if(y>752 && c==1){ //if the pig has hit the ground and is in play
        y=752;//makes pig not in the ground
        }
      }
    }
        // Pipe 1 Scroller
    if (pipe_timer > 0.005 && c==1) {
        pipe_x1 -= 1;
        pipe_x2 -= 1;
        pipe_x3 -= 1;
        pipe_x4 -= 1;
